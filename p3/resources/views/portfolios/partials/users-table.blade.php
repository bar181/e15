            ...
            <tbody>
                @foreach ($bars as $bar)
                    <tr class="border-b transition duration-300 ease-in-out hover:bg-neutral-100">
                        <td class="whitespace-nowrap px-6 py-4 text-left ">{{ $bar->user->name }}</td>
                        <td class="whitespace-nowrap px-6 py-4 text-center">{{ $bar->share_count }}</td>
                        <td class="whitespace-nowrap px-6 py-4 text-center">
                            <a href='/portfolios/{{ $bar->user->id }}'
